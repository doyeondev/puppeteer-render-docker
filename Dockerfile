...
# Puppeteer Docker offers its own Docker image that comes pre-bundled with 
# Chromium and the dependencies required for Puppeteer as well as a pre-installed Puppeteer version
...
# Instead of Docker pull, we're going to say FROM (which is syntax for Docker file)
FROM ghcr.io/puppeteer/puppeteer:22.15.0

# Next, set environment variables
# In Puppeteer docs, we can see that if we want to skip Chromium download when installing Puppeteer, we can do PUPPETEER_SKIP_DOWNLOAD = true
...
# npm ci (i.e. clean install), since we want this to be a repeatable automated build process
RUN npm ci

# Copy the rest of the repository files into our working directory
COPY . . 

# Finally, we will have our 'start command' for our Docker container from package.json (written at package.json > "scripts" > "start": "node index") 
CMD [ "node", "index.js"]

# Now the Dockerfile is completed. Push this change to public git repository
# (git status > git add . > git commit -m "Added Dockerfile" > git push)
# After that, go to scrapeLogic.js